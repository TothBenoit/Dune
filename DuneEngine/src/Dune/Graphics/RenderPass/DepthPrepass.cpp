...
namespace Dune::Graphics
{
	void DepthPrepass::Initialize(Device* pDevice)
	{
		...
		({
			...
			.filePath = L"Shaders\\DepthPrepass.hlsl",
			.entryFunc = L"VSMain",
			.args = args,
	...
	void DepthPrepass::Render(Scene& scene, CommandList& commandList, Camera& camera)
	{
		commandList.SetGraphicsPipeline(m_pipeline);
		commandList.SetPrimitiveTopology(EPrimitiveTopology::TriangleList);

		dMatrix4x4 viewProjectionMatrix;
		ComputeViewProjectionMatrix(camera, nullptr, nullptr, &viewProjectionMatrix);
		commandList.PushGraphicsConstants(0, &viewProjectionMatrix, sizeof(dMatrix4x4));
